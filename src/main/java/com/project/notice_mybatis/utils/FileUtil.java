...
import com.project.notice_mybatis.exception.AttachFileException;
...
//    private final String today = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"));
    ...
    /**
     * 업로드 경로 Paths.get메서드 사용시 파라미터로 전달한 문자열을 연결함.
     */
    private final String uploadPath = Paths.get("C:", "__Programing", "FileUpload", today).toString();
    //uploadPath = "C:"+File.separator+"_Programing"+File.separator+File.separator+"FileUpload"+today;
    ...
    public List<AttachDTO> uploadFiles(MultipartFile[] files, Long boardIdx) {
        ...
        if (dir.exists() == false) {
            //mkdir : 상위 디렉토리 존재 x -> 생성불가.
            //mkdirs : 상위 디렉토리 존재 x -> 상위 디렉토리까지 생성.
            dir.mkdirs();
        }
